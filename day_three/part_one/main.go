	...
	for char_idx, char := range chars {
      ...
      // fmt.Println(char)
			...
			if char != "." {
				...
				symbol := Symbol{coord: symbol_coord}
...
func main() {
	...
	// content, error := os.Open("test_input_two.txt")
	if error != nil {
		log.Fatal(error)
	}
	defer content.Close()
  ...
  all_sum := 0
  for _, number := range engine_schematic.numbers {
    has_neighbour := number.hasNeighbourSymbol(engine_schematic.symbols)
    if has_neighbour {
      all_sum += number.value
      // fmt.Println(number.value)
    } else {
      // fmt.Print(number.value)

    }
    // fmt.Print(has_neighbour)
    // fmt.Println(number)
  }
